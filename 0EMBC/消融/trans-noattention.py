...
class MultiRunOptimizer:
    def __init__(self, patients, machine_exam_map, num_parallel_runs, pop_size_per_run, block_start_date=None):
        ...
        self.step2_pos_window: int = 400
        self.step2_pos_deterministic: bool = False
        # Step2 idx2 选择模式：random（原随机） | attn（注意力） | mask_uniform（仅mask后均匀采样）
        self.step2_pos_pick_mode: str = "random"
        # 记录最近一次 step2 选择（便于外部 RL/日志）
        self.last_step2_rows: torch.Tensor | None = None
        self.last_step2_idx1: torch.Tensor | None = None
        self.last_step2_idx2: torch.Tensor | None = None
    ...
    def enable_step2_mask_only(self, window: int = 400, deterministic: bool = False) -> None:
        """消融：保留 step2 的 hard mask，但去掉注意力选择（cand_mask 内均匀采样）。

        - idx1 仍随机
        - idx2 在 cand_mask==True 的候选中均匀采样（deterministic=True 时取第一个可行）
        """
        self._ensure_gpu_engine()
        self.step2_pos_policy = None
        self.step2_pos_value = None
        self.step2_pos_window = int(window)
        self.step2_pos_deterministic = bool(deterministic)
        self.step2_pos_pick_mode = "mask_uniform"
        self.last_step2_rows = None
        self.last_step2_idx1 = None
        self.last_step2_idx2 = None
        self.last_step2_logprob = None
    ...
    def disable_step2_position_policy(self) -> None:
        self.step2_pos_pick_mode = "random"
        self.step2_pos_policy = None
        self.step2_pos_value = None
        self.last_step2_rows = None
        self.last_step2_idx1 = None
    ...
    def _step2_select_idx2_with_attention(
        ...
        """给定 rows/idx1，为每行选一个 idx2（论文式 attention）。"""
        mode = getattr(self, "step2_pos_pick_mode", None)
        if mode is None:
            mode = "attn" if self.step2_pos_policy is not None else "random"
        if mode == "attn":
            assert self.step2_pos_policy is not None
        if self._patient_main_exam_id is None:
            idx2 = torch.randint(0, N, (rows.numel(),), device=DEVICE)
        ...
        if (self.patient_exam_mask_all is not None) and (self._machine_exam_mask is not None):
        ...
        if self._gpu_engine is not None and hasattr(self._gpu_engine, "has_heart") and hasattr(self._gpu_engine, "has_angio") and hasattr(self._gpu_engine, "has_contrast"):
            # ref_gene -> cand_slot
        ...
        # ===== 消融：mask_uniform（不走注意力，只在 cand_mask 内均匀采样） =====
        if mode == "mask_uniform":
            # fallback：若该行没有任何可行候选，退化为 idx1 的相邻位置
            idx2 = (idx1 + 1) % N
            logprob = None
            vr = torch.nonzero(any_valid, as_tuple=False).flatten()
            if vr.numel() > 0:
                weights = cand_mask[vr].float()  # [Rv,W]  (0/1)
                if deterministic:
                    pick_v = torch.argmax(weights, dim=1)  # 每行取第一个 True
                    logprob = None
                else:
                    pick_v = torch.multinomial(weights, 1, replacement=True).squeeze(1)
                    cnt = weights.sum(dim=1).clamp(min=1.0)
                    logprob = torch.zeros((rows.numel(),), dtype=DTYPE_FLOAT, device=DEVICE)
                    logprob[vr] = -torch.log(cnt)

                idx2_v = cand_pos_safe[vr].gather(1, pick_v.view(-1, 1)).squeeze(1)
                idx2[vr] = idx2_v
...
def main():
    try:
        ...
        _step2_ckpt = os.environ.get("COE_STEP2_POS_CKPT", "").strip()
        _step2_det = os.environ.get("COE_STEP2_POS_DETERMINISTIC", "0").strip() in ("1", "true", "True", "YES", "yes")
        _step2_mask_only = os.environ.get("COE_STEP2_MASK_ONLY", "0").strip() in ("1", "true", "True", "YES", "yes")
        _step2_window = int(os.environ.get("COE_STEP2_POS_WINDOW", "400").strip() or "400")
        if _step2_mask_only:
            print(f"✓ 启用 Step2 mask-only（window={_step2_window}, deterministic={_step2_det}）")
            optimizer.enable_step2_mask_only(window=_step2_window, deterministic=_step2_det)
        elif _step2_ckpt:
            print(f"✓ 启用 Step2 位置选择器（ckpt={_step2_ckpt}, deterministic={_step2_det}）")
            optimizer.enable_step2_position_policy(ckpt_path=_step2_ckpt, deterministic=_step2_det)
            optimizer.step2_pos_window = _step2_window
        else:
            # 如果你希望“无 ckpt 也强制启用并在线训练”，可以在外部显式调用：
            # optimizer.enable_step2_position_policy(ckpt_path=None)
            pass
