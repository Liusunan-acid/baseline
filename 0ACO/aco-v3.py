...
class ACOScheduler(GPUEngineMixin):
    ...
    def _construct_ants_batch(self) -> torch.Tensor:
        """
        无启发式版本：只用 tau 来采样路径（排列）。
        - 起点均匀随机
        - 转移概率 ∝ tau[current, j]^alpha
        """
        N = len(self._idx_to_cid)
        A = self.num_ants

        paths = torch.zeros((A, N), dtype=DTYPE_LONG, device=DEVICE)
        visited = torch.zeros((A, N), dtype=torch.bool, device=DEVICE)

        tau = self._tau  # [N, N]

        # 1) 起点：均匀随机（不使用 eta_urgency）
        current_nodes = torch.randint(low=0, high=N, size=(A,), device=DEVICE, dtype=DTYPE_LONG)
        paths[:, 0] = current_nodes
        visited.scatter_(1, current_nodes.unsqueeze(1), True)

        # 2) 逐步扩展：只用信息素 tau
        for step in range(1, N):
            current_tau = tau[current_nodes]          # [A, N]
            probs = (current_tau ** self.alpha)       # [A, N]
            probs[visited] = 0.0
    ...
    def run_aco(self, max_iter: int | None = None):
        ...
        for it in range(max_iter):
            ...
            # ====== Top-20 递减 + fitness 加权的信息素增强（无启发式版本） ======
            k = min(50, self.num_ants)
            top_vals, top_idx = torch.topk(fitness, k=k, largest=True, sorted=True)  # fitness: [A]
            top_paths = perms_idx[top_idx]  # [k, N]

            # 1) 排名递减权重：k,k-1,...,1 归一化（递减加分）
            ranks = torch.arange(k, device=DEVICE, dtype=DTYPE_FLOAT)  # 0..k-1
            w_rank = (k - ranks)                                       # [k]
            w_rank = w_rank / w_rank.sum()

            # 2) fitness 决定强度：把 top-k 的 fitness 归一化到 [eps, 1]
            f_min = top_vals.min()
            f_max = top_vals.max()
            f_norm = (top_vals - f_min) / (f_max - f_min + 1e-12)      # [0,1]
            f_norm = (f_norm + 1e-3)                                   # 避免为 0

            # 3) 每条路径每条边的增量（由 fitness 决定）
            delta_base = self.q / float(N)
            delta_per_path = delta_base * w_rank * f_norm              # [k]

            from_nodes = top_paths[:, :-1].reshape(-1)                 # [k*(N-1)]
            to_nodes   = top_paths[:, 1:].reshape(-1)                  # [k*(N-1)]
            delta_flat = delta_per_path.repeat_interleave(N - 1)       # [k*(N-1)]
