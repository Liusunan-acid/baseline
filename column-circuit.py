...
# ===================== 新增：circuit 相关配置 =====================
TRANSITION_GAP_SECONDS = 60     # 同机同日序列级换模间隙（秒）
CIRCUIT_MAX_GROUP = 250          # 同机同日候选数超过该阈值则不启用 circuit（避免 O(K^2) 太慢）
USE_TRANSITION_IN_OBJECTIVE = False  # 是否把换模惩罚纳入 CP-SAT 目标（更对齐最终 fitness）
...
class RollingHorizonScheduler:
    ...
    def solve(self):
        total_patients = len(self.all_patients)
        num_workers = multiprocessing.cpu_count()
        print(f"\n🚀 开始滚动优化 (天级精度，按登记时间排序)，启用 {num_workers} 线程")
    ...
    def solve_batch(self, batch_patients, num_workers):
        ...
        ends = {}        # (p_idx, m_id, day_offset) -> IntVar (秒)  # NEW
        p_data = {}      # p_idx -> 病人信息

        # NEW: 记录同机同日有哪些候选 key，避免后面全表扫描
        machine_day_keys = defaultdict(list)  # (m_id, day_offset) -> [key, ...]

        # ===================== NEW: circuit 辅助函数（最小侵入） =====================
        def add_circuit_for_machine_day(m_id, day_offset, keys):
            """
            对同一 (machine, day) 的候选任务建立 circuit，
            并用 arc 触发相邻换模间隙约束。
            返回：用于目标（可选）的“不同类型相邻弧”literal 列表。
            """
            if len(keys) <= 1:
                return []

            # 防止弧爆炸：候选过多时跳过 circuit
            if len(keys) > CIRCUIT_MAX_GROUP:
                print(f"  [circuit] 没有使用circuit：设备 {m_id+1}，day_offset={day_offset} "
                f"候选数={len(keys)} > CIRCUIT_MAX_GROUP={CIRCUIT_MAX_GROUP}")
                return []

            # 建立节点映射：0 为 dummy，其他任务从 1 开始编号
            node_of = {k: i + 1 for i, k in enumerate(keys)}
            key_of_node = {i + 1: k for i, k in enumerate(keys)}

            arcs = []
            diff_type_arc_lits = []

            # dummy self-loop：允许当天该机无任务
            dummy_self = model.NewBoolVar(f"dummy_self_m{m_id}_d{day_offset}")
            arcs.append((0, 0, dummy_self))

            group_pres = [presences[k] for k in keys]
            # dummy_self <-> (sum(pres)==0)
            model.Add(sum(group_pres) == 0).OnlyEnforceIf(dummy_self)
            model.Add(sum(group_pres) >= 1).OnlyEnforceIf(dummy_self.Not())

            # 每个任务节点添加自环与 dummy 连接
            for k in keys:
                i = node_of[k]
                pres_i = presences[k]

                # i->i 自环表示“不选择该任务”
                arcs.append((i, i, pres_i.Not()))

                # 0->i, i->0
                lit_0i = model.NewBoolVar(f"arc_0_{i}_m{m_id}_d{day_offset}")
                lit_i0 = model.NewBoolVar(f"arc_{i}_0_m{m_id}_d{day_offset}")
                arcs.append((0, i, lit_0i))
                arcs.append((i, 0, lit_i0))

                model.AddImplication(lit_0i, pres_i)
                model.AddImplication(lit_i0, pres_i)

            # 任务之间的弧 i->j
            for k_i in keys:
                i = node_of[k_i]
                pres_i = presences[k_i]
                p_i = p_data[k_i[0]]
                exam_i = p_i['exam_type']

                for k_j in keys:
                    if k_i == k_j:
                        continue
                    j = node_of[k_j]
                    pres_j = presences[k_j]
                    p_j = p_data[k_j[0]]
                    exam_j = p_j['exam_type']

                    lit_ij = model.NewBoolVar(f"arc_{i}_{j}_m{m_id}_d{day_offset}")
                    arcs.append((i, j, lit_ij))

                    # arc 被选 => 两端任务都被选
                    model.AddImplication(lit_ij, pres_i)
                    model.AddImplication(lit_ij, pres_j)

                    # 相邻序列约束：start_j >= end_i + setup
                    setup = TRANSITION_GAP_SECONDS if exam_i != exam_j else 0
                    model.Add(starts[k_j] >= ends[k_i] + setup).OnlyEnforceIf(lit_ij)

                    if exam_i != exam_j:
                        diff_type_arc_lits.append(lit_ij)

            model.AddCircuit(arcs)
            return diff_type_arc_lits

        # ===================== 1) 建模：候选位点 + presence + optional interval =====================
        for p_idx, p in enumerate(batch_patients):
            p_data[p_idx] = p
            possible_presences = []
            ...
            for d in range(SEARCH_DAYS):
                ...
                for m_id in range(MACHINE_COUNT):
                    ...
                    intervals[key] = interval_var
                    starts[key] = start_var
                    ends[key] = end_var  # NEW

                    possible_presences.append(is_present)
                    machine_day_keys[(m_id, current_day_offset)].append(key)  # NEW

            # 每个病人必须被安排一次
            if possible_presences:
                model.Add(sum(possible_presences) == 1)
            else:
                print(f"⚠️ 警告: 患者 {p['id']} ({p['exam_type']}) 在 {SEARCH_DAYS} 天内无符合规则的空位")

        # ===================== 2) 每台机每天 NoOverlap（保留原逻辑） =====================
        ...
        # ===================== 3) NEW: 同机同日 circuit + 60 秒换模间隙 =====================
        diff_type_arc_lits_all = []

        # 只对当前批次中出现过候选的 (m, d) 建立
        for (m_id, day_offset), keys in machine_day_keys.items():
            # 只有同机同日真的有两个及以上候选才值得尝试
            if len(keys) >= 2:
                diff_lits = add_circuit_for_machine_day(m_id, day_offset, keys)
                if diff_lits:
                    diff_type_arc_lits_all.extend(diff_lits)

        # ===================== 4) 目标：最小化加权等待天数（可选加入换模惩罚） =====================
        day_costs = []
        for key, is_present in presences.items():
        ...
        if USE_TRANSITION_IN_OBJECTIVE and diff_type_arc_lits_all:
            # 用你原有的 TRANSITION_PENALTY 作为相邻不同类型的软代价
            model.Minimize(sum(day_costs) + TRANSITION_PENALTY * sum(diff_type_arc_lits_all))
        else:
        ...
        if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            ...
            # 更新机器占用表 (秒)
            for (m_id, d_date), ends_list in current_batch_updates.items():
                self.machine_occupied_until[(m_id, d_date)] = max(
                    self.machine_occupied_until[(m_id, d_date)],
                    max(ends_list)
                )
...
def main():
    ...
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_file = os.path.join(current_dir, f'schedule_seconds_fifo_circuit_{ts}.xlsx')
    scheduler.export_excel(out_file, score_data=(score, details))
