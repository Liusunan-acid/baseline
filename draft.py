...
class MultiRunOptimizer:
    ...
    def _mutate_step1_violations(self, X: torch.Tensor, parent_violate_mask: torch.Tensor) -> torch.Tensor:
        """
        X: [C, N]，每行是一个个体的排列（数值是患者 idx）
        parent_violate_mask: [C, N]，现在是“患者级掩码”：
            parent_violate_mask[c, pid] = True 表示该个体中患者 pid 违规
        我们要做的事情是：
            1）为每一行随机选一个“违规患者 pid”
            2）在当前 X 中找到该患者所在的位置 pos
            3）在 pos 附近挑一个随机位置，与之交换
        """
        C, N = X.shape

        # 哪些行有违规患者
        any_viol_per_row = torch.any(parent_violate_mask, dim=1)  # [C]
        viol_rows_idx = torch.nonzero(any_viol_per_row, as_tuple=False).flatten()  # [R]
        R = viol_rows_idx.numel()
        if R == 0:
            return X

        # 只取有违规的那些行的“患者级掩码”
        viol_mask_subset = parent_violate_mask[viol_rows_idx]  # [R, N]，维度 N=患者数

        # 1）在“患者维度”上随机选一个违规患者 pid
        #    每一行至少有一个 True，所以可以直接 multinomial
        viol_patient_id = torch.multinomial(
            viol_mask_subset.float(), 1, replacement=True
        ).flatten()  # [R]，每个元素是 patient_idx

        # 2）在当前个体中找到这个患者所在的位置 pos
        #    X[viol_rows_idx] 的每行是一个排列，等于“当前该行个体的患者顺序”
        rows = X[viol_rows_idx]  # [R, N]，数值同样是 patient_idx

        # 对每一行 r：rows[r, :] == viol_patient_id[r] 只有一个位置为 True
        equal_mask = (rows == viol_patient_id.unsqueeze(1))  # [R, N] bool
        # 由于是排列，每一行恰好一个 True，argmax 就是该患者所在的位置
        viol_pos_idx = equal_mask.float().argmax(dim=1)  # [R]，这里是真正的“位点”

        # 3）在 viol_pos_idx 附近选一个随机位置，与之交换（保持你原来的 ±400 窗口逻辑）
        low = torch.clamp(viol_pos_idx - 400, min=0)
        high = torch.clamp(viol_pos_idx + 400, max=N - 1)
        range_size = high - low + 1

        # 安全：避免 range_size <= 0
        range_size = torch.where(range_size <= 0, torch.ones_like(range_size), range_size)

        rand_offset = torch.floor(torch.rand(R, device=DEVICE) * range_size).long()
        cand_idx_in_row = low + rand_offset  # [R]

        # 避免候选位置和原位置完全相同（且有可用范围时）
        cand_idx_in_row = torch.where(
            (cand_idx_in_row == viol_pos_idx) & (range_size > 1),
            torch.where(viol_pos_idx == low, low + 1, low),
            cand_idx_in_row
        )
        cand_idx_in_row = torch.clamp(cand_idx_in_row, 0, N - 1)

        # 4）交换这两个位置
        val1 = X[viol_rows_idx, viol_pos_idx]
        val2 = X[viol_rows_idx, cand_idx_in_row]
        X[viol_rows_idx, viol_pos_idx] = val2
        X[viol_rows_idx, cand_idx_in_row] = val1
