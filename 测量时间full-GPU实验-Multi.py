...
class MultiRunOptimizer:
    ...
    def _ordered_crossover_batch_gpu(self, P1: torch.Tensor, P2: torch.Tensor) -> torch.Tensor:
        """
        Ordered Crossover (OX) implemented in PyTorch in a vectorized manner.
        """
        DEVICE = parents1.device
        C, N = parents1.shape
        arangeN = torch.arange(N, device=DEVICE).expand(C, N)
        
        # 1. Generate random cut points for each individual in the batch
        a = torch.randint(0, N, (C,), device=DEVICE)
        b = torch.randint(0, N, (C,), device=DEVICE)
        start = torch.minimum(a, b).unsqueeze(1)
        end = torch.maximum(a, b).unsqueeze(1)
        
        # 2. Create mask for the fragment from Parent 1
        mask_frag = (arangeN >= start) & (arangeN < end)
        
        # 3. Initialize children with -1
        children = torch.full_like(parents1, -1, dtype=torch.long)
        
        # 4. Copy fragments from Parent 1
        children[mask_frag] = parents1[mask_frag]
        
        # 5. Fill the rest from Parent 2 (preserving relative order)
        # Check which elements of P2 are already in the P1 fragment
        P2_expanded = parents2.unsqueeze(2)  # (C, N, 1)
        P1_expanded = parents1.unsqueeze(1)  # (C, 1, N)
        mask_frag_expanded = mask_frag.unsqueeze(1)  # (C, 1, N)
        
        # Vectorized check: (P2[i,j] == P1[i,k]) AND mask_frag[i,k]
        # Reduce over k to see if P2 element exists in the fragment
        equality_matrix = (P2_expanded == P1_expanded)
        isin_matrix = (equality_matrix & mask_frag_expanded).any(dim=2)
        
        # Elements in P2 that are NOT in the P1 fragment
        mask_tail = ~isin_matrix
        
        # Extract valid elements from P2 (flattened per batch effectively)
        P2_tails_indices = arangeN[mask_tail]
        
        # Create a mask for the positions to fill in the children
        mask_fill = (children == -1)
        
        # Use gather to fill the children tensor with the valid elements from P2
        children[mask_fill] = P2_tails_indices[mask_fill]
        
        # Ensure each child is a valid permutation
        for i in range(C):
            _, perm_indices = torch.sort(children[i])
            children[i] = perm_indices
