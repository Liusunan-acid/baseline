...
class MultiRunOptimizer:
    ...
    def evolve_gpu(self, generations=100, elite_size=5):
        ...
        for gen_idx in range(generations):
            ...
            # 4) 交叉 + 定点变异（改：先修父代，再交叉；子代不再走 step1）
            ...
            if num_children > 0:
                ...
                # --- 新增：取“对应父代”的患者级违规mask（保证患者维）---
                Vmask_p1 = torch.gather(parent_viol, 1, p_idx1.unsqueeze(2).expand(-1, -1, N))  # [K,nc,N]
                Vmask_p2 = torch.gather(parent_viol, 1, p_idx2.unsqueeze(2).expand(-1, -1, N))  # [K,nc,N]

                # 展平
                C = self.K * num_children
                P1_flat = P1.view(C, N).contiguous()
                P2_flat = P2.view(C, N).contiguous()
                V1_flat = Vmask_p1.view(C, N).contiguous()
                V2_flat = Vmask_p2.view(C, N).contiguous()

                # --- 核心修改：对“参与生成子代的父代”先做患者级定点修复 ---
                # 只会从 mask=True 的 pid 中抽样，因此定点修复一定落在“违规患者”上（患者级）
                P1_flat = self._mutate_step1_violations(P1_flat, V1_flat, n_swaps_per_individual=1)
                P2_flat = self._mutate_step1_violations(P2_flat, V2_flat, n_swaps_per_individual=1)

                # 有序交叉（用修复后的父代）
                children_flat = self._ordered_crossover_batch_gpu(P1_flat, P2_flat)

                # --- 子代只做 step2/step3，避免再次用“父代mask”去修子代导致错配 ---
                children_flat = self._mutate_step2_base_swap(children_flat, self.current_generation, base_swap_prob=0.95)
                children_flat = self._mutate_step3_greedy_cluster(children_flat, greedy_prob=0.5)
